...
public class ChunkProviderCookieverse implements IChunkProvider {
    ...
    public ChunkProviderCookieverse(World par1World, long par2, boolean par4)
    {
                    ...
                    for (int l2 = 0; l2 < 8; ++l2)
                    {
                        ...
                        for (int i3 = 0; i3 < 4; ++i3)
                        {
                            ...
                            for (int k3 = 0; k3 < 4; ++k3)
                            {
                                if ((d15 += d16) > 0.0D)
                                {
                                    p_147424_3_[j3 += short1] = net.minecraft.init.Blocks.stone;
                                }
                                else if (k2 * 8 + l2 < b0)
                                {
                                    p_147424_3_[j3 += short1] = net.minecraft.init.Blocks.water;
                                }
                                else
                                {
    ...
    public Chunk provideChunk(int par1, int par2)
    {
		...
		for(int x = 0; x < 16; ++x) {
			for(int z = 0; z < 16; ++z) {
				for(int y = 0; y < 256; ++y) {
					Block block = ablock[x << 12 | z << 8 | y];
					BiomeGenBase biome = biomesForGeneration[x*16+z];
					if(biome instanceof BiomeGenCookieverse) {
						Block mainBlock = ((BiomeGenCookieverse)biome).mainBlock;
						if(block == Blocks.grass) {
							...
							block = Blocks.milkStill;
						}
						 else if(block == Blocks.lava) {
							block = Blocks.hotChocolateStill;
						}
    ...
    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
    {
                ...
                && TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, flag, LAKE))
        {
            ...
            (new WorldGenLakes(Blocks.milkStill)).generate(this.worldObj, this.rand, k1, l1, i2);
        }
        ...
        if (TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, flag, LAVA) && !flag && this.rand.nextInt(8) == 0)
        {
            ...
            {
                (new WorldGenLakes(Blocks.hotChocolateStill)).generate(this.worldObj, this.rand, k1, l1, i2);
            }
        }
        ...
        for (k1 = 0; doGen && k1 < 16; ++k1)
        {
            for (l1 = 0; l1 < 16; ++l1)
            {
                ...
                {
                    this.worldObj.setBlock(k1 + k, i2 - 1, l1 + l, net.minecraft.init.Blocks.ice, 0, 2);
                }

                if (this.worldObj.func_147478_e(k1 + k, i2, l1 + l, true))
                {
                    this.worldObj.setBlock(k1 + k, i2, l1 + l, net.minecraft.init.Blocks.snow_layer, 0, 2);
                }
            }
        }
