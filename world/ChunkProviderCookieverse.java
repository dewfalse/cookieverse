...
IChunkProvider {
	...
	public Chunk provideChunk(int par1, int par2)
	{
		...
		for(int x = 0; x < 16; ++x) {
			for(int z = 0; z < 16; ++z) {
				for(int y = 0; y < abyte.length / 256; ++y) {
					int blockID = abyte[x << 11 | z << 7 | y];
					BiomeGenBase biome = biomesForGeneration[x*16+z];
					if(biome instanceof BiomeGenCookieverse) {
						int mainBlockID = ((BiomeGenCookieverse)biome).mainBlockID;
						if(blockID == Block.grass.blockID) {
							blockID = (short) mainBlockID;
						}
						else if(blockID == Block.dirt.blockID) {
							blockID = (short) Blocks.blackChocolateCookieDough.blockID;
						}
							...
							blockID = (short) Blocks.blackChocolateCookie.blockID;
						}
						else if(blockID == Block.sand.blockID) {
							blockID = (short) Blocks.cookieDough.blockID;
						}
		...
		Chunk chunk = new Chunk(this.worldObj, ids, metadata, par1, par2);
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
	{
				...
				!flag && this.rand.nextInt(8) == 0)
		{
			...
			{
				(new WorldGenLakes(Blocks.hotChocolateStill.blockID)).generate(this.worldObj, this.rand, k1, l1, i2);
			}
		}
